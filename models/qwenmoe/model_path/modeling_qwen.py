...
from .qwen2_config import Qwen2MoeConfig
# import expertcachewithscorefullcpu
# from models.qwenmoe import qwen_module

# from models.qwemoe.qwen_module import compute_time_1
# from ..qwen_module import compute_time_1
...
class Qwen2MoeRMSNorm(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        # global tokens
        # if tokens > 0:
        #     logger.info('model Qwen2MoeRMSNorm forward: %s',end - start)
...
class Qwen2MoeSdpaAttention(Qwen2MoeAttention):
    ...
    def forward(
        ...
        if past_key_value is not None:
            cache_kwargs = {"sin": sin, "cos": cos, "cache_position": cache_position}  # Specific to RoPE models
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx,cache_kwargs) #if_update
        ...
        # logger.info('moesdpa attention: %s', end-start)
...
class Qwen2MoeDecoderLayer(nn.Module):
    def __init__(self, config: Qwen2MoeConfig, layer_idx: int):
        ...
        if (layer_idx not in config.mlp_only_layers) and ((layer_idx + 1) % config.decoder_sparse_step == 0):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.mlp(hidden_states,residual,self_attn_weights,present_key_value,attention_mask,position_ids,output_attentions,cache_position,position_embeddings)
...
class Qwen2MoeModel(Qwen2MoePreTrainedModel):
    ...
    def forward(
        ...
        pre_foward = time.time()
        # global tokens
        # if tokens > 0:
        #     logger.info('model pre forward: %s',pre_foward - s)
        for decoder_layer in self.layers:
            # logger.info('decoder_layer: %s', decoder_layer)
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = decoder_layer( #计算1
                    hidden_states,
        ...
        global decode_time
        global CPUtime
        global tops,lows,pcie_load_time,top_loads,low_loads
        if tokens==0:
            logger.warning("prefill time %s",e-s)
            pcie_load_time = 0
            CPUtime = 0
            tokens+=1
        else:
            decode_time+=(e-s)
            tokens+=1
            logger.info("pcie load time %s",pcie_load_time)
            logger.info("CPUtime %s",CPUtime)
#            logger.info("topratio %s",tops/(tops+lows))
#            logger.info("toploadratio %s",top_loads/tops)
#            logger.info("lowloadratio %s",low_loads/lows)
            logger.info("decode time of a token %s",(e-s))
            pcie_load_time = 0
            CPUtime = 0
        
            if tokens == 96:
                logger.warning("decode time of a token %s",(e-s))
                # logger.warning("decode time of a token eval %s",decode_time/tokens)
