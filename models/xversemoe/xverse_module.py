...
# scoretime=0
# prefetch_hit_sum=0
from .model_path import modeling_xverse
...
class XverseMoEMLPwithCache(nn.Module):
    def __init__(
    ...
    def forward(self, hidden_states,residual_cur,attn_weights_cur,present_key_value_cur,attention_mask,position_ids,output_attentions,cache_position):
        ...
        routing_weights, selected_experts = torch.topk(Scores, self.top_k, dim=-1, sorted=True)
        ...
        #     printindex = [(self.layerid,i) for i in sort_index[0]]
        #     print(printindex)
        ...
        replaceset = []
        # print("selected_expertsbefore",selected_experts)
        if self.replaceScoreRatio!=None:
            replaceset = replaceset_between_tokens(Scores.tolist(),self.replaceScoreRatio,self.top_k)
        ...
        for expert_idx in range(self.num_experts):
            ...
            if current_state.size()[0]>3:
                experttokens3+=1
        # logger.info("experttokens3ratio %s",experttokens3/64)
        #根据expert_token_dic和self.expertcache运算得到final_hidden_states
        self.moe_infer_for_one_batch(replaceset,final_hidden_states,expert_token_dic,hidden_states.dtype,residual_cur,attn_weights_cur,present_key_value_cur,hidden_states,attention_mask,position_ids,output_attentions,cache_position,(batch_size,sequence_length,hidden_dim))
        # else:
        #     expert_token_dic=dict()
    ...
    def compute_expertsincache_in_thread(self,expertcomputeusecudaincache,x, expert_token_dic,hdtype,expert_num_not_in_cuda_num,residual_cur,attn_weights_cur,present_key_value_cur,identity,attention_mask,position_ids,output_attentions,expert_out_dict,cache_position,bsh):
        # 这部分是在新线程中执行的for循环逻辑
        s= time.time()
        self.next_experts = None
        ...
        for uid in expertcomputeusecudaincache:
            ...
            logger.debug("computetimecudainnew_thread %s", endcom - startcom, uid)
        ...
        if self.layerid <27 and expert_num_not_in_cuda_num>0 and self.if_prefetch and modeling_xverse.tokens>0:
            s=time.time()
            self.next_experts = self.get_next_top_expert(residual_cur,attn_weights_cur,present_key_value_cur,hidden_states,identity,attention_mask,position_ids,output_attentions,cache_position,bsh)
            e =time.time()
    ...
    @torch.no_grad()
    def moe_infer_for_one_batch(self, replaceset,final_hidden_states, expert_token_dic,hdtype,residual_cur,attn_weights_cur,present_key_value_cur,identity,attention_mask,position_ids,output_attentions,cache_position,bsh):
        logger.debug("--------------")
        s= time.time()
        ...
        # logger.debug("beforequeue %s",self.ExpertCache.load_queue)
        ...
        # logger.debug("topratio %s",len(newreplace)/18)
        for uid in expert_token_dic.keys():
            if not self.ExpertCache.query_expert(uid):
                ...
                if modeling_xverse.tokens > 0:
                    # global top_loads,low_loads
                    if uid in newreplace:
                        modeling_xverse.top_loads+=1
                    else:
                        modeling_xverse.low_loads+=1
        ...
        thread = threading.Thread(target=self.compute_expertsincache_in_thread, args=(
            ...
            residual_cur,attn_weights_cur,present_key_value_cur,identity,attention_mask,position_ids,output_attentions,expert_out_dict,cache_position,bsh
        ))
        thread.start()
        ...
        for uid in expertcomputeusecpu:
            ...
            if modeling_xverse.tokens>0:
                modeling_xverse.CPUtime+=(endcom-startcom)
        ...
        for uid in expertneedload:
            ...
            expert_out = expert(expert_tokens)
            endcom=time.time()
            logger.debug("computetimecuda %s",endcom-startcom,uid)
            weight= expert_token_dic[uid][1]
            expert_out.mul_(weight)
    ...
    @torch.no_grad()
    def get_next_top_expert(self,residual_cur,attn_weights_cur,present_key_value_cur,hidden_states,raw_hidden,attention_mask,position_ids,output_attentions,cache_position,bsh):
        #         final_hidden_states = final_hidden_states.reshape(batch_size, sequence_length, hidden_dim)
        ...
        hidden_states = residual_cur+hidden_states # sit3, formal
        ...
        logger.debug("next_hidden_states %s",hidden_states.size())
        logits = self.next_gate_weight(next_hidden_states)
        logger.debug("logits %s",logits.size())
        ...
        topexperts = replaceset_between_tokens(scores.tolist(),self.replaceScoreRatio,self.top_k)
        # logger.info("next experts tops %s, layer %s",topexperts, self.layerid+1)
        return topexperts