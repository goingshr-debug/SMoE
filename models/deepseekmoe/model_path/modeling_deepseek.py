...
class DeepseekSdpaAttention(DeepseekAttention):
    ...
    def forward(
        ...
        bsz, q_len, _ = hidden_states.size()
        logger.debug("hidden_states.size() %s",hidden_states.device)
        query_states = self.q_proj(hidden_states)
        key_states = self.k_proj(hidden_states)
        value_states = self.v_proj(hidden_states)
        ...
        if past_key_value is not None:
            cache_kwargs = {"sin": sin, "cos": cos}  # Specific to RoPE models
            key_states, value_states = past_key_value.update(key_states, value_states, self.layer_idx,if_update, cache_kwargs)
            # else:
            #     a=past_key_value.get_usable_length(kv_seq_len, self.layer_idx)
            #     rawpast_key_value = copy.deepcopy(past_key_value)
            #     key_states, value_states = rawpast_key_value.update(key_states, value_states, self.layer_idx, cache_kwargs)
            #     a=past_key_value.get_usable_length(kv_seq_len, self.layer_idx)
        e1 = time.time()
        if if_update:
            logger.debug("attentionp1nopre %s",e1-s)
        else:
            logger.debug("attentionp1pre %s",e1-s)
        key_states = repeat_kv(key_states, self.num_key_value_groups)
        value_states = repeat_kv(value_states, self.num_key_value_groups)
...
class DeepseekDecoderLayer(nn.Module):
    def __init__(self, config: DeepseekConfig, layer_idx: int):
        ...
        logger.debug(config._attn_implementation)
    ...
    def forward(
        ...
        e1 = time.time()
        logger.debug("timeofattention %s %s",self.layer_idx,e1-s1)
        if 1<=self.layer_idx <=27:
            hidden_states,_ = self.mlp(hidden_states,residual,self_attn_weights,present_key_value,attention_mask,position_ids,output_attentions)
        else:
        ...
        e2 = time.time()
        logger.debug("timeexceptffn %s %s",self.layer_idx,e2-s2)
        return outputs
...
class DeepseekModel(DeepseekPreTrainedModel):
    ...
    def forward(
        ...
        logger.debug(hidden_states.size())
        if not return_dict:
        ...
        global decode_time
        if tokens==0:
            logger.info("prefill time %s",e-s)
            expertcachewithscorefullcpu.pcie_load_time = 0
            tokens+=1
        else:
            decode_time+=(e-s)
            tokens+=1
            logger.info("pcie load time %s",expertcachewithscorefullcpu.pcie_load_time)
            logger.info("decode time of a token %s",(e-s))
            
            if tokens == 32:
                expertcachewithscorefullcpu.pcie_load_time = 0
                logger.debug("decode time of a token %s",(e-s))
                logger.debug("decode time of a token eval %s",decode_time/tokens)
